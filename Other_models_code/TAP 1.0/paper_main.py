...
data_positive=features("positive.txt")
label_positive=np.ones(data_positive.shape[0])
data_negative=features("negative.txt")
...
train_data,test_data,train_label,test_label=train_test_split(data,label,random_state=0,train_size=0.7)
"""-----------------------------十折交叉验证---------------------------------"""
k=10
np.random.seed(1234)
...
for fold in range(k):
    ...
    trainFeature=train_data[mode!=fold]
    testFeature=train_data[mode==fold]
...
np.savez("data3-model3-train.npz",predict=score_prob,label=train_label)      
...
test_predict_pro=model.predict_proba(test_data)[:,1]
...
print(f"ACC:{acc}\nAUC:{auc}\nRecall:{recall}\nPrecision:{precision}\nF1:{f1}\nKappa:{kappa}\nMCC:{mcc}")
np.savez("data3-model3-test.npz",predict=test_predict_pro,label=test_label)        

# -*- coding: utf-8 -*-
"""
Created on Fri Jul 22 10:16:14 2022
...
def cnn(max_len,depth,l1=16,l2=256,gamma=1e-4,lr=8e-4,w1=3,w2=2):
    ...
    model.add(layers.Dense(2,activation='softmax',kernel_regularizer=regularizers.l2(gamma)))
...
def read_data(lines):
    ...
    return seq,np.array(label).astype("float32")
...
test_label=smooth_labels(test_label)
f.close()
...
for fold in range(k):
    ...
    m1.fit(trainFeature1,trainLabel,batch_size=32,epochs=200,verbose=1)
    score_aaindex_cnn[mode==fold]=m1.predict(testFeature1).reshape(len(testFeature1),2)
    
np.savez('cnntrain.npz',cnn=score_aaindex_cnn,label=train_label)

model=cnn(padding_lens,31)
model.fit(train_encoding,train_label,batch_size=32,epochs=200,verbose=1,validation_data=(test_encoding,test_label),
          shuffle=True)
test_preidct=model.predict(test_encoding).reshape(len(test_encoding),2)

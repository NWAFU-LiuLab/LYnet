# -*- coding: utf-8 -*-
"""
Created on Thu Jul 14 18:37:25 2022
...
train_label=get_label(r"seqset/Train.FASTA")
...
test_label=get_label(r"seqset/Test.FASTA")
...
for i in range(len(train_label)):
    if score_RF[i]==train_label[i]:
        right=right+1
        
print("Ten fold validation acc:%lf"%(right/len(train_label)))
print("Ten fold validation AUC:%lf"%roc_auc_score(train_label,score_RF_prob))
"""-------------------------------------------------------------------------"""

best_estimator.fit(best_train_features,train_label)
score=best_estimator.score(best_test_features,test_label)
print("indepandent test准确率为：\n",score)

test_predict=best_estimator.predict_proba(best_test_features)[:,1]

auc=roc_auc_score(test_label,test_predict)
print("indepandent test AUC指标：\n",auc)

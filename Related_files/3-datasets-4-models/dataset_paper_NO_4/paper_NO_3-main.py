...
def features(file):
    ...
    f.close()
    return np.array(aaindex_values),np.array(label)


data_train,train_label=features("Train.FASTA")
data_test,test_label=features("Test.FASTA")
...
np.savez("data4-model3-train.npz",predict=score_prob,label=train_label)            
...
print(f"ACC:{acc}\nAUC:{auc}\nRecall:{recall}\nPrecision:{precision}\nF1:{f1}\nKappa:{kappa}\nMCC:{mcc}")
np.savez("data4-model3-test.npz",predict=test_predict_pro,label=test_label)        

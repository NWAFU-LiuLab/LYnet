...
#positive_GPSD
positive_AAC=AAC(r"Positive.fasta")
positive_CTDC=libsvm_file_read(r"positive_CTDC.LibSVM")
positive_CTDT=libsvm_file_read(r"positive_CTDT.LibSVM")
positive_CTDD=libsvm_file_read(r"positive_CTDD.LibSVM")
#positive_GAAPC
positive_GAAC=GAAC(r"Positive.fasta")
positive_GDPC=libsvm_file_read(r"positive_GDPC.LibSVM")
positive_GTPC=libsvm_file_read(r"positive_GTPC.LibSVM")
#positive_ASDC
positive_ASDC=libsvm_file_read(r"positive_ASDC.LibSVM")
#positive_PAAC
positive_PAAC=libsvm_file_read(r"positive_PAAC.LibSVM")

positive_feature=np.concatenate((positive_AAC,positive_CTDC,positive_CTDT,positive_CTDD,
                              positive_GAAC,positive_GDPC,positive_GTPC,
                              positive_ASDC,positive_PAAC),axis=1)


#negative_GPSD
negative_AAC=AAC(r"Negative.fasta")
negative_CTDC=libsvm_file_read(r"negative_CTDC.LibSVM")
negative_CTDT=libsvm_file_read(r"negative_CTDT.LibSVM")
negative_CTDD=libsvm_file_read(r"negative_CTDD.LibSVM")
#negative_GAAPC
negative_GAAC=GAAC(r"Negative.fasta")
negative_GDPC=libsvm_file_read(r"negative_GDPC.LibSVM")
negative_GTPC=libsvm_file_read(r"negative_GTPC.LibSVM")
#negative_ASDC
negative_ASDC=libsvm_file_read(r"negative_ASDC.LibSVM")
#negative_PAAC
negative_PAAC=libsvm_file_read(r"negative_PAAC.LibSVM")

negative_feature=np.concatenate((negative_AAC,negative_CTDC,negative_CTDT,negative_CTDD,
                              negative_GAAC,negative_GDPC,negative_GTPC,
                              negative_ASDC,negative_PAAC),axis=1)
...
train_feature,test_feature,train_label,test_label=train_test_split(feature,label,train_size=0.8,random_state=22)
...
np.savez("data3-model4-train.npz",predict=score_RF_prob,label=train_label)        
print("最佳估计器: \n",estimator.best_estimator_)
print("最佳参数:\n",estimator.best_params_)
...
np.savez("data3-model4-test.npz",predict=test_predict_pro,label=test_label)

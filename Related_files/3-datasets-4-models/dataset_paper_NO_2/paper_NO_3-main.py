...
data_train_positive=features("Train_Positive.fasta")
data_train_negative=features("Train_Negative.fasta")
data_test_positive=features("Test_Positive.fasta")
data_test_negative=features("Test_Negative.fasta")


scale=MinMaxScaler()

data_train=np.concatenate((data_train_positive,data_train_negative),axis=0)
train_label=np.concatenate((np.ones(data_train_positive.shape[0]),np.zeros(data_train_negative.shape[0])),axis=0)

data_test=np.concatenate((data_test_positive,data_test_negative),axis=0)
test_label=np.concatenate((np.ones(data_test_positive.shape[0]),np.zeros(data_test_negative.shape[0])),axis=0)
...
for fold in range(k):
    ...
    trainFeature=data_train[mode!=fold]
    testFeature=data_train[mode==fold]
...
np.savez("data2-model3-train.npz",predict=score_prob,label=train_label)            
...
test_predict_pro=model.predict_proba(data_test)[:,1]
...
print(f"ACC:{acc}\nAUC:{auc}\nRecall:{recall}\nPrecision:{precision}\nF1:{f1}\nKappa:{kappa}\nMCC:{mcc}")
np.savez("data2-model3-test.npz",predict=test_predict_pro,label=test_label)        
